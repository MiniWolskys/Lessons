...
def create_Ray(O, D):
    Ray = {'Origine': np.array(O), 'Direction' : np.array(D)}
    return Ray 

def create_sphere(C, r, a, d, s, cr, i):
...
def create_plane(P, n, a, d, s, cr, i):
    plane = {'Type': 'plan', 'Position':  np.array(P), 'Normale': np.array(n), 'Ambient' : np.array(a), 'Diffuse' : np.array(d), 'Specular' : np.array(s), 'Reflexion': float(cr), 'Index': int(i) }
    return plane
